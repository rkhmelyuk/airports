...
<head>
    ...
    <script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?key=AIzaSyBpb1bsIkVLntYfrBR8ls5ZRcmK6G5whoc&&sensor=false"></script>
...
<body>
<h1>${fieldValue(bean: airport, field: 'name')}</h1><hr/>
    ...
    <div class="pull-left">
        ...
        <g:if test="${airport.webSite != ""}">
            <div class="field clearfix-after">
                ...
                <div class="value"><g:link action="show" base="${airport.webSite}">${fieldValue(bean: airport, field: 'webSite')}</g:link></div>
            </div>
        </g:if>
    </div>
...
<g:if test="${airportImages}">
    ...
    <div class="main-side pull-left">
        <img src="${createLink(controller: 'image', action: 'getMidImage', id: airportImages[0].id)}" class="main-image"/>
    </div>
